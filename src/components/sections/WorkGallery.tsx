...
import CurieFhirPipeline from "@/components/sections/CurieFhirPipeline";
      ...
      <div className="order-2 flex flex-col rounded-xl border border-border bg-surface/50 p-5 sm:p-6 md:p-8 lg:order-1 lg:min-h-[640px]">
        ...
        <div className="mt-2 flex items-center gap-2.5">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            aria-label="CurieAI source on GitHub"
...
function CurieFhirProjectBlock() {
  return (
    <article className="mx-auto mt-10 w-full max-w-6xl rounded-xl border border-border bg-surface/50 p-5 sm:mt-12 sm:p-6 md:p-8 xl:max-w-7xl">
      <p className="text-xs uppercase tracking-wide text-muted">{curieFhir.eyebrow}</p>
      <div className="mt-2 flex items-center gap-2.5">
        <h2 className="font-serif text-2xl leading-snug text-foreground md:text-3xl">
          <a
            href={curieFhir.githubUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="underline decoration-border underline-offset-4 transition-colors hover:decoration-foreground"
          >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          aria-label="CurieFHIR source on GitHub"
          className="text-muted transition-colors hover:text-foreground"
        >
          <GitHubIcon size={20} />
        </a>
      </div>
      <p className="mt-1.5 text-sm text-muted">{curieFhir.tagline}</p>
      <p className="mt-4 max-w-3xl text-sm leading-relaxed text-foreground md:text-base">
        {curieFhir.description}
      </p>

      <CurieFhirPipeline />

      <ul className="mt-5 grid gap-3 sm:grid-cols-2">
        {curieFhir.highlights.map((highlight) => (
          <li key={highlight.label} className="text-sm leading-relaxed text-muted">
            <span className="font-medium text-foreground">{highlight.label}</span>
            {" — "}
            {highlight.text}
